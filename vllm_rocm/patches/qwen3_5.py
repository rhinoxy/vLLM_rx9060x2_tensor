...
from vllm.model_executor.layers.layernorm import RMSNorm as Qwen3_5RMSNorm
...
class Qwen3_5ForCausalLMBase(
    ...
    def compute_logits(
        ...
        if logits is not None and logits.numel() > 0:
            topk_vals, topk_inds = torch.topk(logits[-1:], k=10, dim=-1)
            print(f"[DEBUG LOGITS] hs_shape={hidden_states.shape} hs_norm={hidden_states.norm().item():.3f} logits_shape={logits.shape} min={logits.min().item():.2f} max={logits.max().item():.2f} topk_inds={topk_inds[0].tolist()} topk_vals={topk_vals[0].tolist()}", flush=True)
        return logits
