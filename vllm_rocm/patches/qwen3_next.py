...
class Qwen3NextDecoderLayer(nn.Module):
    def __init__(
        ...
        if self.layer_idx in (57, 58, 62, 63):
            print(f'[DETAILED L{self.layer_idx}] after mlp: min={hidden_states.min().item():.3f}, max={hidden_states.max().item():.3f}, std={hidden_states.std().item():.3f}', flush=True)
        if self.layer_scale:
    ...
    def forward(
        ...
        **kwargs: object,
        ...
        if self.layer_idx == 0:
            print(f"[ENTRY L0 DETAILED] hs_norm={hidden_states.norm().item():.3f} hs_mean={hidden_states.mean().item():.4f} hs_std={hidden_states.std().item():.4f} hs_sample={hidden_states[0, :5].tolist()}", flush=True)
        if self.layer_idx in (0, 1):
            print(f'[ENTRY L{self.layer_idx}] hidden_states nan: {torch.isnan(hidden_states).any().item()}, residual nan: {torch.isnan(residual).any().item() if residual is not None else False}', flush=True)
        ...
        if _in_nan:
            print(f'[NAN_TRACE] L{self.layer_idx} INPUT is already NaN!', flush=True)
        if self.layer_type == "linear_attention":
            hidden_states = self.linear_attn(hidden_states=hidden_states)
            if self.layer_idx == 56 and torch.isnan(hidden_states).any():
                print(f'[DEBUG NaN] L56 linear_attn produced NaN!', flush=True)
        elif self.layer_type == "full_attention":
        ...
        if self.layer_idx in (0, 4):
            print(f'[L{self.layer_idx} ATTR] after attn: nan={torch.isnan(hidden_states).any().item()}, min={hidden_states.min().item()}, max={hidden_states.max().item()}', flush=True)
        if not _in_nan and torch.isnan(hidden_states).any().item():
            print(f'[NAN_TRACE] L{self.layer_idx} produced NaN in ATTN ({self.layer_type})!', flush=True)
        if self.layer_scale:
        ...
        if self.layer_idx == 1:
            print(f'[L1] before post_norm hidden_states nan: {torch.isnan(hidden_states).any().item()}, residual nan: {torch.isnan(residual).any().item()}', flush=True)
        if self.layer_idx in (57, 58, 62, 63):
            print(f'[DETAILED L{self.layer_idx}] after attn: min={hidden_states.min().item():.3f}, max={hidden_states.max().item():.3f}, std={hidden_states.std().item():.3f}, res_min={residual.min().item():.3f}, res_max={residual.max().item():.3f}', flush=True)
        # Fully Connected
        hidden_states, residual = self.post_attention_layernorm(hidden_states, residual)
        if self.layer_idx in (57, 58, 62, 63):
            print(f'[DETAILED L{self.layer_idx}] after post_norm: min={hidden_states.min().item():.3f}, max={hidden_states.max().item():.3f}, std={hidden_states.std().item():.3f}', flush=True)
        if self.layer_idx == 1:
            print(f'[L1] after post_norm hidden_states nan: {torch.isnan(hidden_states).any().item()}', flush=True)
        if self.use_attn_reduce_scatter_for_moe:
            ...
            if self.layer_idx == 1:
                print(f'[L1] after mlp hidden_states nan: {torch.isnan(hidden_states).any().item()}', flush=True)
            if self.layer_idx == 56 and torch.isnan(hidden_states).any():
                print(f'[DEBUG NaN] L56 mlp produced NaN!', flush=True)
        ...
        if not _in_nan and (torch.isnan(hidden_states).any().item() or (residual is not None and torch.isnan(residual).any().item())):
            print(f'[NAN_TRACE] L{self.layer_idx} produced NaN in MLP/POST!', flush=True)
        if self.layer_idx >= 50:
            h_std = hidden_states.std().item()
            h_mean = hidden_states.mean().item()
            print(f'[LAYER_STATS L{self.layer_idx}] mean={h_mean:.4f}, std={h_std:.4f}, min={hidden_states.min().item():.4f}, max={hidden_states.max().item():.4f}', flush=True)
...
class Qwen3NextModel(nn.Module, EagleModelMixin):
    ...
    def forward(
        ...
        for layer_idx, layer in enumerate(
            ...
            if True:
                print(f"[LAYER_TRACE] L{layer_idx} type={layer.layer_type} hs_norm={hidden_states.norm().item():.3f} res_norm={residual.norm().item() if residual is not None else 0:.3f}", flush=True)
            if torch.isnan(hidden_states).any():
                print(f'[DEBUG NaN] NaN detected at layer {layer_idx}!', flush=True)
