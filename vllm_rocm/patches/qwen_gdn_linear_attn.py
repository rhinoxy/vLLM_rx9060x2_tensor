...
class QwenGatedDeltaNetAttention(GatedDeltaNetAttention):
    ...
    def _output_projection(
        ...
        if any(x in getattr(self, 'prefix', '') for x in ('layers.0.', 'layers.4.')):
            print(f'[OUT_PROJ L0/4] z nan: {torch.isnan(z).any().item()}, z min={z.min().item()}, z max={z.max().item()}', flush=True)
            print(f'[OUT_PROJ L0/4] core_attn_out before norm nan: {torch.isnan(core_attn_out).any().item()}', flush=True)
        core_attn_out = self.norm(core_attn_out, z)
        if any(x in getattr(self, 'prefix', '') for x in ('layers.0.', 'layers.4.')):
            print(f'[OUT_PROJ L0/4] core_attn_out after norm nan: {torch.isnan(core_attn_out).any().item()}', flush=True)
        core_attn_out = core_attn_out.reshape(z_shape_og)
        core_attn_out = core_attn_out.flatten(-2)  # ... h d -> ... (h d)
        output, _ = self.out_proj(core_attn_out)
        if any(x in getattr(self, 'prefix', '') for x in ('layers.0.', 'layers.4.')):
            print(f'[OUT_PROJ L0/4] out_proj output nan: {torch.isnan(output).any().item()}', flush=True)
        return output
    ...
    def forward_cuda(
        ...
        # ============================================================
        if any(x in getattr(self, 'prefix', '') for x in ('layers.0.', 'layers.4.')):
            print(f'[GDN L0/4 BEFORE] hidden_states nan: {torch.isnan(hidden_states).any().item()}, min={hidden_states.min().item()}, max={hidden_states.max().item()}', flush=True)
        mixed_qkvz, _ = self.in_proj_qkvz(hidden_states)
        ba, _ = self.in_proj_ba(hidden_states)
        if any(x in getattr(self, 'prefix', '') for x in ('layers.0.', 'layers.4.')):
            print(f'[GDN L0/4] mixed_qkvz nan: {torch.isnan(mixed_qkvz).any().item()}', flush=True)
            print(f'[GDN L0/4] ba nan: {torch.isnan(ba).any().item()}', flush=True)
        ...
        if any(x in getattr(self, 'prefix', '') for x in ('layers.0.', 'layers.4.')):
            print(f'[GDN L0/4 DETAILED] mixed_qkv nan={torch.isnan(mixed_qkv).any().item()}, b nan={torch.isnan(b).any().item()}, a nan={torch.isnan(a).any().item()}, z nan={torch.isnan(z).any().item()}', flush=True)
        ...
        if any(x in getattr(self, 'prefix', '') for x in ('layers.0.', 'layers.4.')):
            print(f'[GDN L0/4 DETAILED] after core op: core_attn_out nan={torch.isnan(core_attn_out).any().item()}, min={core_attn_out.min().item()}, max={core_attn_out.max().item()}', flush=True)
        if any(x in getattr(self, 'prefix', '') for x in ('layers.0.', 'layers.4.')):
            print(f'[GDN L0/4] core_attn_out nan: {torch.isnan(core_attn_out).any().item()}', flush=True)
    ...
    def _forward_core(
        ...
        if attn_metadata.num_prefills > 0:
            ...
            if any(f'layers.{l}.' in getattr(self, 'prefix', '') for l in (0, 4)):
                print(f'[CORE {self.prefix}] after conv1d: nan={torch.isnan(mixed_qkv_non_spec).any().item()}', flush=True)
        ...
        if attn_metadata.num_prefills > 0:
            ...
            if any(f'layers.{l}.' in getattr(self, 'prefix', '') for l in (0, 4)):
                print(f'[CORE {self.prefix}] A_log min={self.A_log.min().item()} max={self.A_log.max().item()} nan={torch.isnan(self.A_log).any().item()}', flush=True)
                print(f'[CORE {self.prefix}] dt_bias min={self.dt_bias.min().item()} max={self.dt_bias.max().item()} nan={torch.isnan(self.dt_bias).any().item()}', flush=True)
                print(f'[CORE {self.prefix}] a_prefill min={a_prefill.min().item()} max={a_prefill.max().item()} nan={torch.isnan(a_prefill).any().item()}', flush=True)
                print(f'[CORE {self.prefix}] b_prefill min={b_prefill.min().item()} max={b_prefill.max().item()} nan={torch.isnan(b_prefill).any().item()}', flush=True)
                print(f'[CORE {self.prefix}] after prep: q_nan={torch.isnan(query_non_spec).any().item()} k_nan={torch.isnan(key_non_spec).any().item()} v_nan={torch.isnan(value_non_spec).any().item()} g_nan={torch.isnan(g_non_spec).any().item()} beta_nan={torch.isnan(beta_non_spec).any().item()} g_min={g_non_spec.min().item()} g_max={g_non_spec.max().item()} beta_min={beta_non_spec.min().item()} beta_max={beta_non_spec.max().item()}', flush=True)
        ...
        if attn_metadata.num_prefills > 0:
            ...
            if any(f'layers.{l}.' in getattr(self, 'prefix', '') for l in (0, 4)):
                print(f'[CORE {self.prefix}] chunk_gated_delta_rule backend={self.chunk_gated_delta_rule} init_state_nan={torch.isnan(initial_state).any().item()}', flush=True)
            (
            ...
            if any(f'layers.{l}.' in getattr(self, 'prefix', '') for l in (0, 4)):
                print(f'[CORE {self.prefix}] after chunk_gated_delta_rule: out_nan={torch.isnan(core_attn_out_non_spec).any().item()} last_rec_nan={torch.isnan(last_recurrent_state).any().item()}', flush=True)
            # Init cache
